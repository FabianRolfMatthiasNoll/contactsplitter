    ...
    for pattern in exclusion_patterns:
        if pattern.startswith("/") and pattern.endswith("/"):
            ...
            if fnmatch.fnmatch(path, pattern) or any(
                fnmatch.fnmatch(part, pattern) for part in path.split(os.sep)
            ):
                return True
    return False
            ...
            for file in files:
                ...
                if file_types is None or any(file.endswith(ext) for ext in file_types):
                    ...
                    try:
                        ...
                        print(
                            f"Error reading file {file_rel_path}: {str(e)}. Skipping."
                        )
                        out_file.write(
                            f"Error reading file: {str(e)}. Content skipped.\n"
                        )
...
def main(args: List[str]) -> None:
    if len(args) < 3:
        print(
            "Usage: python script.py <start_path> <output_file> [exclusion_file] [file_extensions...]"
        )
    ...
    exclusion_patterns = (
        parse_exclusion_file(exclusion_file) if exclusion_file else set()
    )
